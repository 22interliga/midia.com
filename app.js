...
function slaPage(){
 const today=todayISO(); const start=new Date();start.setDate(start.getDate()-7);const startISO=start.toISOString().slice(0,10);
...
function renderSlaResults(){
 let a=$('#slaStart')?.value,b=$('#slaEnd')?.value,driver=$('#slaDriver')?.value||'',base=$('#slaBase')?.value||'',local=($('#slaLocal')?.value||'').toLowerCase();
 let rows=state.orders.filter(o=>(!a||o.prazo>=a)&&(!b||o.prazo<=b)&&(!driver||o.entregador===driver)&&(!base||o.base===base)&&(!local||String(o.localidade||o.cidade).toLowerCase().includes(local)));
 let total=rows.length, delivered=rows.filter(o=>o.status==='Entregue').length, late=rows.filter(o=>slaText(o)==='Atrasado').length, occ=rows.filter(o=>o.status==='Ocorrência').length, onTime=rows.filter(o=>slaText(o)!=='Atrasado').length;
 let pct=total?Math.round(onTime/total*100):0;
 $('#slaResult').innerHTML=`<div class="grid kpis">${kpi('SLA',pct+'%','Dentro do prazo')}${kpi('Pacotes',total,'No filtro selecionado')}${kpi('Entregues',delivered,'Baixas concluídas')}${kpi('Atrasados',late,'SLA vencido','danger')}${kpi('Ocorrências',occ,'Insucessos','warn')}</div><div class="card" style="margin-top:18px"><h3>Detalhamento do período</h3>${table(rows,false)}</div>`;
}
...
function render(p='dashboard'){
 let item=navItems.find(x=>x[0]===p)||navItems[0];$('#pageTitle').textContent=item[2];$('#pageSubtitle').textContent=item[3];const pageFn=pages[item[0]]||pages.dashboard; $('#content').innerHTML=pageFn();$$('.nav-btn').forEach(b=>b.classList.toggle('active',b.dataset.page===item[0]));history.replaceState(null,'','#'+item[0]);
 let s=$('#search');if(s)s.oninput=e=>{let q=e.target.value.toLowerCase();$$('tbody tr').forEach(tr=>tr.style.display=tr.innerText.toLowerCase().includes(q)?'':'none')};
 if(item[0]==='sla'){renderSlaResults();$('#slaApply').onclick=renderSlaResults;$('#slaClear').onclick=()=>{let st=new Date();st.setDate(st.getDate()-7);$('#slaStart').value=st.toISOString().slice(0,10);$('#slaEnd').value=todayISO();$('#slaDriver').value='';$('#slaBase').value='';$('#slaLocal').value='';renderSlaResults()}}
 if(item[0]==='motoristas')$('#addDriver').onclick=()=>openDriver();
 if(item[0]==='bases')$('#addBase').onclick=()=>openBase();
 if(item[0]==='despesas')$('#addExpense').onclick=()=>{fillDialogOptions();$('#expenseForm').reset();$('#expenseForm [name=data]').value=todayISO();$('#expenseDialog').showModal()};
 if(item[0]==='usuarios')$('#addUser').onclick=()=>{fillDialogOptions();$('#userForm').reset();$('#userDialog').showModal()};
 if(item[0]==='ia')bindAI();
}

$('#nav').innerHTML=navItems.map(x=>`<button class="nav-btn" data-page="${x[0]}"><span>${x[1]}</span><div><strong>${x[2]}</strong><small>${x[3]}</small></div></button>`).join('');
$('#nav').onclick=e=>{let b=e.target.closest('.nav-btn');if(b){render(b.dataset.page);closeMenu()}};
...
$('#newBtn').onclick=()=>{fillDialogOptions();$('#wbForm').reset();$('#wbForm [name=prazo]').value=todayISO();$('#wbDialog').showModal()};
$$('.close').forEach(b=>b.onclick=()=>$('#wbDialog').close());$$('.close-occ').forEach(b=>b.onclick=()=>$('#occDialog').close());$$('.close-driver').forEach(b=>b.onclick=()=>$('#driverDialog').close());$$('.close-expense').forEach(b=>b.onclick=()=>$('#expenseDialog').close());$$('.close-user').forEach(b=>b.onclick=()=>$('#userDialog').close());$$('.close-base').forEach(b=>b.onclick=()=>$('#baseDialog').close());
...
window.birthdayMessage=id=>{let d=state.drivers.find(x=>x.id===id);let msg=`Feliz aniversário, ${d.nome.split(' ')[0]}! Desejamos um excelente dia, muita saúde, felicidade e sucesso. 🎉`;navigator.clipboard?.writeText(msg);alert(msg+'\n\nMensagem copiada. O envio automático por WhatsApp será conectado na etapa de backend/API.')};

function bindAI(){
 $$('.ai-q').forEach(b=>b.onclick=()=>{$('#aiQuestion').value=b.dataset.q;runAI(b.dataset.q)});$('#aiAsk').onclick=()=>runAI($('#aiQuestion').value)
}
function runAI(q){q=(q||'').toLowerCase();let ans='';if(q.includes('sla')){let late=state.orders.filter(o=>slaText(o)==='Atrasado');ans=`Há ${late.length} volume(s) com SLA vencido em ${state.orders.length} registros. ${late.length?'Priorize '+[...new Set(late.map(x=>x.base))].join(', ')+'.':'Não há atraso no conjunto atual.'}`}
 else if(q.includes('base')){let stats=state.bases.map(b=>{let a=state.orders.filter(o=>o.base===b.nome),late=a.filter(o=>slaText(o)==='Atrasado').length;return {b:b.nome,t:a.length,l:late}}).sort((a,b)=>b.l-a.l);ans=stats.map(x=>`${x.b}: ${x.t} pacote(s), ${x.l} atrasado(s)`).join('\n')}
 ...
 else {let late=state.orders.filter(o=>slaText(o)==='Atrasado').length,occ=state.orders.filter(o=>o.status==='Ocorrência').length;ans=`Resumo operacional: ${state.orders.length} Waybill(s), ${late} atrasado(s), ${occ} ocorrência(s) e ${state.drivers.filter(d=>d.status==='Ativo').length} motorista(s) ativo(s). Para uma análise mais específica, mencione SLA, base, motorista ou ocorrências.`}
